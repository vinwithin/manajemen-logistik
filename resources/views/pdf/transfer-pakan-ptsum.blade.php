...
<head>
    <meta charset="utf-8">
    <title>Transfer Pakan {{ $from ?? 'Semua' }} - {{ $to ?? 'Semua' }} - PT Sum</title>
    <style>
        * {
...
<body>
    ...
    @php
        ...
        $cvLogo = $cv?->logo;
        $cvLogoBase64 = null;
        if ($cvLogo && \Illuminate\Support\Facades\Storage::disk('public')->exists($cvLogo)) {
            $type = pathinfo($cvLogo, PATHINFO_EXTENSION);
            $data = \Illuminate\Support\Facades\Storage::disk('public')->get($cvLogo);
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 8px;">
        <tr>
            ...
            <td style="width: 44%; vertical-align: middle; text-align: center; padding: 0;">
                <div class="doc-title">REKAPITULASI TRANSFER PAKAN</div>
    ...
    <table class="rekap">
        ...
        <tbody>
            ...
            @foreach ($headers as $header)
                @foreach ($header->kendaraans->sortBy('no_polisi') as $kendaraan)
                        ...
                        @foreach ($penerima->pakans as $pakan)
                            ...
                            <tr class="{{ $rowClass }} isi">
                                ...
                                <td style="vertical-align:middle;">
                                    {{ $header->tanggal_transfer->translatedFormat('d F Y') }}
                                </td>
                                ...
                                <td class="td-center">{{ Str::upper($header->nama_pengirim ?? '-') }}</td>
            ...
            <tr class="row-total">
                <td colspan="7" style="text-align:center; letter-spacing:1px;">TOTAL</td>
