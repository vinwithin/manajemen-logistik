...
<body>
    ...
    <div class="doc-title">No. PO : {{ $po->no_po }} &nbsp;&mdash;&nbsp; {{ $po->tanggal_po->format('d/m/Y') }}
        @if ($po->cv)
            &nbsp;&mdash;&nbsp; {{ $po->cv->nama_cv }}
        @endif
    </div>
    ...
    <table class="rekap">
        <thead>
            <tr class="head-group">
                ...
                <th rowspan="2" style="width:38px;">ONGKOS OA<br>(Rp/kg)</th>
                <th rowspan="2" style="width:55px;">TOTAL OA<br>(Rp)</th>
            </tr>
            <tr class="head-sub">
                @foreach ($kodePakanList as $kp)
                    <th style="min-width:30px;">{{ $kp->kode }}</th>
                @endforeach
                @foreach ($kodePakanList as $kp)
                    <th style="min-width:30px;">{{ $kp->kode }}</th>
                @endforeach
            ...
            @foreach ($po->kendaraans->sortBy('no_polisi') as $kendaraan)
                ...
                @foreach ($kendaraan->penerimas as $penerima)
                    ...
                    <tr class="{{ $rowClass }}">
                        ...
                        <td class="td-left">{{ $penerima->tujuan?->nama ?? '-' }}</td>
    ...
    @php
        ...
        $noNota = $cvPrefix ? $cvPrefix . '/SUP/' . $po->tanggal_po->format('Y/m') : 'SUP/' . $po->no_po;
        $totalNota = 0;
        $notaRows = [];
        $noRow = 1;
        foreach ($po->kendaraans->sortBy('no_polisi') as $kendaraan) {
    ...
    <div class="nota-wrap">
            ...
            <tr>
                ...
                <td style="width:45%; vertical-align:middle;">
                    <table class="nota-yth">
                        ...
                        <tr>
                            <td class="nota-yth-label">No. PO</td>
                            <td style="width:10px;"></td>
                            <td class="nota-yth-line">{{ $po->no_po }} &mdash;
        ...
        <table class="nota-sign-table">
            <tr>
                ...
                <td style="width:33%;">
                    <span class="nota-sign-city">Jambi, {{ $po->tanggal_po->format('d-M-y') }}</span>
