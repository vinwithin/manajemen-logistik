...
<head>
    <meta charset="utf-8">
    <title>Lansir Gudang - Supplier</title>
    <style>
        * {
            margin: 18px;
            padding: 0;
...
<body>
    ...
    <table class="rekap">
        <thead>
            <tr class="head-group">
                ...
                <th rowspan="2" style="width:38px;">ONGKOS OA(Rp/kg)</th>
                <th rowspan="2" style="width:55px;">TOTAL OA (Rp)</th>
            </tr>
            <tr class="head-sub">
                ...
                @foreach ($header->kendaraans as $kendaraan)
                    ...
                    @foreach ($kendaraan->penerimas as $penerima)
                        ...
                        <tr class="{{ $rowClass }} isi">
                            ...
                            @foreach ($kodePakanList as $i => $kp)
                                @php
                                    ...
                                    if ($pakan && $pakan->jumlah_kg) {
                                        ...
                                        $totalKg += $pakan->jumlah_kg;
                                        $totalOngkos += $pakan->jumlah_kg * ($pakan->ongkos_oa ?? 0);
                            ...
                            <td class="td-total">
                                {{ $totalOngkos > 0 ? number_format($totalOngkos, 0, ',', '.') : '-' }}</td>
                        </tr>
                        @php
                            $grandTotalKg += $totalKg;
                            $grandTotalOngkos += $totalOngkos;
                        @endphp
                    @endforeach
                @endforeach
            @endforeach
    ...
    <table class="sign-table">
        <tr>
            <td><span class="sign-label">Diverifikasi Oleh, <br>Finance Accounting</span><span
                    class="sign-line">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
            </td>
            <td><span class="sign-label">Diketahui Oleh, <br>Branch Manager</span><span
                    class="sign-line">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
            </td>
            <td><span class="sign-label">{{ now()->translatedFormat('d F Y') }}</span><span
                    class="sign-line">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
            </td>
        </tr>
    </table>
    ...
    @php
        // Ambil CV dari field cv_id di header (field baru), fallback ke PO penerima
        $cv = $headers->first()?->cv;
        if (!$cv) {
            $firstPenerima = $headers->first()?->kendaraans->first()?->penerimas->first();
            $cv = $firstPenerima?->poPenerima?->kendaraan?->po?->cv ?? null;
        }
        $cvNama = $cv?->nama_cv ?? '-';
        $cvAlamat = $cv?->alamat ?? '-';
        ...
        foreach ($headers as $header) {
            foreach ($header->kendaraans as $kendaraan) {
                foreach ($kendaraan->penerimas as $penerima) {
                    ...
                    $ongkos = $penerima->pakans->where('ongkos_oa', '>', 0)->first()?->ongkos_oa ?? 0;
                    $total = $totalKg * $ongkos;
                    $notaRows[] = [
                        'no' => $noRow++,
                        'tgl' => $header->tanggal_lansir->format('d-M-Y'),
    ...
    <div class="nota-wrap">
        <table class="nota-header-table">
            <tr>
                <td style="width:55%;">
                    ...
                    <div style="font-size:9px; font-weight:bold; color:rgb(123,123,239);">{{ $cvNama }}</div>
        ...
        <table class="nota-sign-table">
            <tr>
                <td style="width:33%;"><span style="display:block; margin-bottom:50px;">Disetujui,</span><span
                        class="nota-sign-line">(&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;)</span>
                </td>
                <td style="width:33%;"><span style="display:block; margin-bottom:50px;">Diperiksa,</span><span
                        class="nota-sign-line">(&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;)</span>
                </td>
                <td style="width:33%;">
