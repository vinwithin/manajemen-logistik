...
@section('content')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Input PO Baru</h5>
                </div>
                <div class="card-body">
                    {{-- Error dari session (database error, dll) --}}
                    @if (session('error'))
                    ...
                    {{-- Error summary dari server (validation errors) --}}
                    @if ($errors->any())
                        <div class="alert alert-danger alert-dismissible fade show mb-4" role="alert">
                    ...
                    <form method="post" action="{{ route('purchase-order.store') }}" id="formPO">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-4">
                                        <label class="form-label">No. PO dari PT SUM <span
                                                class="text-danger">*</span></label>
                                        <input type="text" name="no_po"
                                            class="form-control @error('no_po') is-invalid @enderror"
                                            value="{{ old('no_po') }}" placeholder="Contoh: PO-2026-001">
                                        @error('no_po')
                                        ...
                                        <select name="cv_id" id="selectCv"
                                            ...
                                            @foreach ($userCvs as $cv)
                                                <option value="{{ $cv->id }}" data-omzet="{{ $cv->omzet_tahun }}"
                                                    ...
                                                    {{ old('cv_id', $activeCvId) == $cv->id ? 'selected' : '' }}
                        ...
                        <div class="mt-4">
                            ...
                            <a href="{{ route('purchase-order.index') }}" class="btn btn-secondary">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="card mb-3 item-kendaraan border-primary">
            ...
            <div class="card-body">
                ...
                <div class="border rounded p-3 bg-light">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="fw-semibold small text-primary">Penerima <span class="text-muted fw-normal">(opsional
                                — bisa diisi nanti)</span></span>
    ...
    <template id="tmplPakan">
        <div class="row g-2 align-items-end mb-2 item-pakan">
            <div class="col-md-4">
                <label class="form-label small">Kode Pakan</label>
    ...
    <script>
        ...
        function addKendaraan(oldData) {
            ...
            if (oldData) {
                $card.find('[name="kendaraan[' + ki + '][no_polisi]"]').val(oldData.no_polisi || '');
                $card.find('[name="kendaraan[' + ki + '][nama_sopir]"]').val(oldData.nama_sopir || '');
                $card.find('[name="kendaraan[' + ki + '][no_surat_jalan]"]').val(oldData.no_surat_jalan || '');
                if (oldData.jumlah_kg) {
                    $card.find('.input-muatan-kg').val(oldData.jumlah_kg);
                    updateMuatanKarung($card);
                }

                // Set DP fields
                if (oldData.dp_nominal) {
                    var dpNominal = parseFloat(oldData.dp_nominal) || 0;
                    $card.find('.input-dp-nominal').val(dpNominal);
                    // Format untuk display (defined in po-dp-handler.js)
                ...
                if (oldData.dp_tanggal) {
                    $card.find('[name="kendaraan[' + ki + '][dp_tanggal]"]').val(oldData.dp_tanggal);
                }
                if (oldData.dp_metode) {
                    $card.find('[name="kendaraan[' + ki + '][dp_metode]"]').val(oldData.dp_metode);
                }
                if (oldData.dp_keterangan) {
                    $card.find('[name="kendaraan[' + ki + '][dp_keterangan]"]').val(oldData.dp_keterangan);
                }

                // Set supplier dan populate jenis kendaraan dropdown
                if (oldData.supplier_id) {
                ...
                var penerimas = oldData.penerima || [];
        ...
        // ── Add penerima ──────────────────────────────────────────
        function addPenerima($kendaraanCard, ki, pi, oldData) {
            ...
            if (oldData) {
                // Set penerima_id dan nama_penerima di dropdown
                var $select = $card.find('.input-nama-penerima');
                $select.val(oldData.nama_penerima || '');
                $card.find('.input-penerima-id').val(oldData.penerima_id || '');
                // Set tujuan dari data yang ada
                if (oldData.tujuan_id) {
                    $card.find('.input-tujuan-id').val(oldData.tujuan_id);
                    var tujuanNama = $select.find('option[data-tujuan-id="' + oldData.tujuan_id + '"]').first().data(
                        'tujuan-nama') || '';
        ...
        // ── Add pakan ─────────────────────────────────────────────
        function addPakan($penerimaCard, ki, pi, pki, oldData) {
            var $row = $(buildPakan(ki, pi, pki));
            $penerimaCard.find('.list-pakan').append($row);

            if (oldData) {
                $row.find('[name*="[kode_pakan_id]"]').val(oldData.kode_pakan_id || '');
                $row.find('.input-jumlah-kg').val(oldData.jumlah_kg || '');
                $row.find('[name*="[ongkos_oa]"]').val(oldData.ongkos_oa || 0);
        ...
        function updateKarung($input) {
            ...
            // Setelah update karung pakan, recalc indikator kendaraan
        ...
        function updateIndikatorMuatan($kCard) {
            ...
            // Hitung total kg dari semua pakan semua penerima di kendaraan ini
            var totalPenerima = 0;
            ...
            if (Math.abs(selisih) < 0.01) {
                $ind.removeClass('alert-warning alert-danger').addClass('alert-success')
                    .html('<i class="fa fa-check-circle"></i> <strong>Cocok!</strong> Total pakan penerima (' +
                        totalPenerima.toLocaleString('id-ID') + ' kg) = muatan kendaraan.')
                    .show();
            } else if (selisih > 0) {
                $ind.removeClass('alert-success alert-danger').addClass('alert-warning')
                    .html('<i class="fa fa-exclamation-triangle"></i> Sisa muatan belum dialokasikan: <strong>' +
                        ...
                        ' kg, penerima ' + totalPenerima.toLocaleString('id-ID') + ' kg)')
                    .show();
            } else {
                $ind.removeClass('alert-success alert-warning').addClass('alert-danger')
                    .html('<i class="fa fa-times-circle"></i> Total pakan penerima <strong>melebihi</strong> muatan: ' +
                        'kelebihan <strong>' + Math.abs(selisih).toLocaleString('id-ID') + ' kg</strong>')
                    .show();
            }
        }
        ...
        // ── Dropdown penerima: native select (tanpa select2) ─────
        function initSelect2Penerima($select) {
            // Native select — tidak perlu inisialisasi khusus
        }
        ...
        // ── Load Jenis Kendaraan ketika Supplier dipilih ─────────────
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (oa > 0) {
                            ...
                            $pCard.find('.oa-info-badge').css('display', 'inline-flex !important').show();
                        }
        ...
        $('#formPO').on('submit', function(e) {
            ...
            if (!$('[name="no_po"]').val().trim()) {
                $('[name="no_po"]').addClass('is-invalid');
                valid = false;
            }
            if (!$('[name="tanggal_po"]').val()) {
                $('[name="tanggal_po"]').addClass('is-invalid');
                valid = false;
            }
            ...
            $kendaraans.each(function() {
                ...
                // Penerima dan pakan bersifat opsional — tidak divalidasi di sini
            });
        ...
        @if (old('kendaraan'))
            @php
                $oldKendaraan = collect(old('kendaraan'))
                    ->map(function ($kend) {
                        return [
                            'no_polisi' => $kend['no_polisi'] ?? '',
                            'nama_sopir' => $kend['nama_sopir'] ?? '',
                            'no_surat_jalan' => $kend['no_surat_jalan'] ?? '',
                            'supplier_id' => $kend['supplier_id'] ?? '',
                            'jumlah_kg' => $kend['jumlah_kg'] ?? '',
                            'dp_nominal' => $kend['dp_nominal'] ?? 0,
                            'dp_tanggal' => $kend['dp_tanggal'] ?? '',
                            'dp_metode' => $kend['dp_metode'] ?? '',
                            'dp_keterangan' => $kend['dp_keterangan'] ?? '',
                            'penerima' => array_values(
                    ...
                    ->toArray();
            @endphp
            var oldKendaraan = @json($oldKendaraan);
            oldKendaraan.forEach(function(k) {
                addKendaraan(k);
            });

            // Highlight field yang error dari server
            @if ($errors->any())
                @foreach ($errors->keys() as $key)
                    @php
                        // Ubah dot notation ke bracket notation: kendaraan.0.no_polisi → kendaraan[0][no_polisi]
