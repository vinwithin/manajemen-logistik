  <div id="sidebar">
      <div class="sidebar-wrapper active">
          ...
          <div class="sidebar-menu">
              {{-- CV Switcher --}}
              @if (isset($userCvs) && $userCvs->count() > 0)
                  <div class="px-3 py-2 border-bottom">
                      <div class="dropdown">
                          <button
                              class="btn btn-sm w-100 d-flex align-items-center justify-content-between
                          {{ $activeCv ? 'btn-outline-primary' : 'btn-outline-secondary' }}"
                              type="button" data-bs-toggle="dropdown" aria-expanded="false">
                              <span class="d-flex align-items-center gap-2">
                                  @if ($activeCv)
                                      <span class="avatar avatar-xs bg-primary text-white rounded"
                                          style="width:24px;height:24px;font-size:10px;display:inline-flex;align-items:center;justify-content:center;">
                                          {{ strtoupper(substr($activeCv->nama_cv, 0, 2)) }}
                                      </span>
                                      <span class="text-truncate"
                                          style="max-width:130px">{{ $activeCv->nama_cv }}</span>
                                  @else
                              ...
                              @foreach ($userCvs as $cv)
                                  <li>
                                      <form action="{{ route('switch.cv') }}" method="POST">
                                          ...
                                          <button type="submit"
                                              class="dropdown-item d-flex align-items-center gap-2
                                      {{ $activeCv && $activeCv->id === $cv->id ? 'active' : '' }}">
                                              <span
                                                  class="avatar avatar-xs rounded text-white
                                          {{ $activeCv && $activeCv->id === $cv->id ? 'bg-primary' : 'bg-secondary' }}"
                                                  style="width:20px;height:20px;font-size:9px;display:inline-flex;align-items:center;justify-content:center;">
              ...
              <ul class="menu">
                  ...
                  @foreach ($sidebarMenus as $menu)
                      ...
                      @else
                          ...
                          <li class="sidebar-item has-sub {{ $isChildActive ? 'active' : '' }}">
                              <a href="#" class="sidebar-link {{ $isChildActive ? 'active' : '' }}">
